...
type
  ...
  TItem = class
    ...
    function AddBVolume(AVolume: PBoundingVolume): PBoundingVolume; virtual;
    function GetBVByIndex(BVIndex: Integer; var PrevVolume: PBoundingVolume): PBoundingVolume; virtual;
    procedure DeleteBVolume(Index: Integer); virtual;
...
begin
  ...
  DebugManager := nil;
  BoundingVolumes.TotalVolumes := 0;
  CurrentLOD := nil;
  Manager := nil;
...
function TItem.AddBVolume(AVolume: PBoundingVolume): PBoundingVolume;
begin
  Result := AVolume;
  with BoundingVolumes do begin
    Inc(TotalVolumes);
    if First = nil then First := AVolume else Last.Next := AVolume;
    Last := AVolume;
  end;
  CalcDimensions;
end;

function TItem.GetBVByIndex(BVIndex: Integer; var PrevVolume: PBoundingVolume): PBoundingVolume;
begin
  PrevVolume := nil;
  Result := BoundingVolumes.First;
  while (Result <> nil) and (BVIndex > 0) do begin
    Dec(BVIndex);
    PrevVolume := Result;
    Result:= Result.Next;
  end;
...
procedure TItem.DeleteBVolume(Index: Integer);
var PrevVolume, CurVolume: PBoundingVolume;
begin
  if (Index < 0) or (Index > BoundingVolumes.TotalVolumes-1) then Exit;
  CurVolume := GetBVByIndex(Index, PrevVolume);
  if PrevVolume <> nil then PrevVolume.Next := CurVolume.Next;
  if BoundingVolumes.Last = CurVolume then BoundingVolumes.Last := PrevVolume;
  if BoundingVolumes.First = CurVolume then BoundingVolumes.First := BoundingVolumes.First.Next;
  FreeMem(CurVolume);
  Dec(BoundingVolumes.TotalVolumes);
end;

procedure TItem.CalcDimensions;
var i: Integer; t: PBoundingVolume; BBMin, BBMax, tv: TVector3s;
begin
// World.Log('Calculating dimensions of item "' + Name + '"'); 
  with BoundingVolumes do begin
    Dimensions := GetVector3s(0, 0, 0);
    t := First;
    if t <> nil then begin
      BBMin := SubVector3s(t.Offset, t.Dimensions);
      BBMax := AddVector3s(t.Offset, t.Dimensions);
      t := First.Next;
      while t <> nil do begin
        tv := SubVector3s(t.Offset, t.Dimensions);
        BBMin := GetVector3s(MinS(BBMin.X, tv.X), MinS(BBMin.Y, tv.Y), MinS(BBMin.Z, tv.Z));
        tv := AddVector3s(t.Offset, t.Dimensions);
        BBMax := GetVector3s(MaxS(BBMax.X, tv.X), MaxS(BBMax.Y, tv.Y), MaxS(BBMax.Z, tv.Z));
        t := t.Next;
      end;
      Dimensions := ScaleVector3s(SubVector3s(BBMax, BBMin), 0.5);
    end;
  end;
  ...
  FullBoundingBox.P1 := BBMin; FullBoundingBox.P2 := BBMax;
end;
...
var i: Integer; T, OldT: PBoundingVolume;
begin
...
//    Result.CurrentLOD := CurrentLOD;
  ...
  with Result.BoundingVolumes do begin
    TotalVolumes := BoundingVolumes.TotalVolumes;
    OldT := BoundingVolumes.First;
    for i := 0 to TotalVolumes-1 do begin
      GetMem(T, SizeOf(TBoundingVolume));
      T^ := OldT^;
      OldT := OldT^.Next;
      T^.Next := nil;
      if i = 0 then First := T else Last^.Next := T;
      Last := T;
    end;
  end;
...
var
  ...
  s, ResName: TShortName; Sign: TFileSignature; T: PBoundingVolume;
begin
  Result := feCannotRead;
...
//      Exit;
    ...
    with BoundingVolumes do begin
      ...
      for i := 0 to TotalVolumes-1 do begin
        GetMem(T, SizeOf(TBoundingVolume));
        Stream.Read(T^, SizeOf(T^)-SizeOf(T^.Next));
        T^.Next := nil;
        if i = 0 then First := T else Last^.Next := T;
        Last := T;
      end;
...
var i, TRes: Integer; T: PBoundingVolume; s: TShortName; Loc: TVector3s;
begin
  Result := feCannotWrite;
  ...
  with BoundingVolumes do begin
    ...
    T := First;
    for i := 0 to TotalVolumes-1 do begin
      Stream.Write(T^, SizeOf(T^)-SizeOf(T^.Next));
      T := T^.Next;
    end;
  end;
...
begin
  ...
  if Assigned(Parent) then begin
    ModelMatrix := MulMatrix4s(ModelMatrix, Parent.ModelMatrix);
    BoundingVolumes.Location := CutVector3s(Transform4Vector4s(Parent.ModelMatrix, ExpandVector3s(Location)));
  end else BoundingVolumes.Location := Location;
  BoundingVolumes.Matrix := CutMatrix3s(ModelMatrix);
...
begin
  ...
  SetLength(Childs, 0); TotalChilds := 0;
  DisposeBoundingVolumes(BoundingVolumes);
  ClearMeshes;
end;
...
begin
  ...
  for i := 0 to TotalMeshes-1 do if (Meshes[i].ClassName = AMesh.ClassName) and                // ToFix: Add resource dependency cheking
//                                    (Meshes[i].VerticesRes = AMesh.VerticesRes) and (Meshes[i].IndicesRes = AMesh.IndicesRes) and
...
begin
  ...
  if CurrentLOD <> nil then begin
    ...
    BoundingBox.P1 := ScaleVector3s(BoundingVolumes.Dimensions, -1);
    BoundingBox.P2 := BoundingVolumes.Dimensions;
  end;
{  CalcDimensions;
  ...
  BoundingVolumes.Location := Location;
  BoundingVolumes.Matrix := CutMatrix3s(ModelMatrix);
end;
...
var Temp: TMatrix4s; BVolume: PBoundingVolume;
begin
  BVolume := BVol.First;
  while BVolume <> nil do begin
    Inc(TotalItems); SetLength(Items, TotalItems); SetLength(Matrices, TotalItems);
    Items[TotalItems-1] := BVolume^.VolumeKind-1;
    Matrices[TotalItems-1] := Matrix;
    Temp := TranslationMatrix4s(BVolume.Offset.X, BVolume.Offset.Y, BVolume.Offset.Z);
    Matrices[TotalItems-1] := MulMatrix4s(Temp, Matrices[TotalItems-1]);
    Temp := ScaleMatrix4s(BVolume.Dimensions.X, BVolume.Dimensions.Y, BVolume.Dimensions.Z);
    Matrices[TotalItems-1] := MulMatrix4s(Temp, Matrices[TotalItems-1]);
...
var i, Height: Integer; CR: TCollisionResult; ShellBoundingVolumes: TBoundingVolumes; Dist, Factor: Single;
  XAxis, YAxis, ZAxis: TVector3s;
  tmat, RevMat: TMatrix3s;
begin
  Result := False;
  HitStatus := hsNone; HitItem := nil;
  if FirstProcess then begin
    FirstProcess := False;
    LastLocation := Location;
    Exit;
  end;
  if not World.EditorMode then begin
//    LastLocation := GetVector3s(0, 0, 0);
//    Location := GetVector3s(100, 0, 0); 

    SubVector3s(ZAxis, Location, LastLocation);

    ShellBoundingVolumes.Location := AddVector3s(LastLocation, ScaleVector3s(ZAxis, 0.5*0));
...
//    ShellBoundingVolumes.Dimensions.Y := 30;
    ...
    ShellBoundingVolumes.First := NewBoundingVolume(vkOOBB, GetVector3s(0, 0, Dist*0.5), ShellBoundingVolumes.Dimensions);
    ShellBoundingVolumes.Last := ShellBoundingVolumes.First;

{    Assert(not SphereOOBBColDet(IdentityMatrix3s, ShellBoundingVolumes.Matrix, AddVector3s(Location, ScaleVector3s(SubVector3s(Location, LastLocation), 0.1)), LastLocation,
                     NewBoundingVolume(vkSphere, GetVector3s(0, 0, 0), GetVector3s(1, 1, 1))^,
                     ShellBoundingVolumes.First^));
    Assert(SphereOOBBColDet(IdentityMatrix3s, ShellBoundingVolumes.Matrix, Location, LastLocation,
                     NewBoundingVolume(vkSphere, GetVector3s(0, 0, 0), GetVector3s(1, 1, 1))^,
                     ShellBoundingVolumes.First^));}

    if CDObjects then for i := 0 to World.TotalItems - 1 do if World.Items[i].ID <> ID then begin
      CR := VolumeColDet(ShellBoundingVolumes, World.Items[i].BoundingVolumes);
      if CR.Vol1 <> nil then begin
        if CreateEvents then World.Events.Add(cmdCollision, [LongInt(Self), LongInt(World.Items[i])]);
        HitStatus := HitStatus or hsItem; HitItem := World.Items[i];
      end;
    end;

    DisposeBoundingVolumes(ShellBoundingVolumes);

    if CDLand and (World.Landscape.HeightMap <> nil) then with World.Landscape.HeightMap do begin
      if (Location.Y + BoundingVolumes.Dimensions.Y > -BreakHeight) then begin         // The shell isn't under break
        Height := GetHeight(Location.X, Location.Z);
        Dist := Location.Y - BoundingVolumes.Dimensions.Y - Height;
        if (Dist < 0) and (Height >= MinHeight) then begin
          ...
          while (Abs(Dist) > LandCollisionAcc) and (Factor < 1) do begin
            Dist := XAxis.Y - BoundingVolumes.Dimensions.Y - GetHeight(XAxis.X, XAxis.Z);
...
begin
  ...
  if Location.X < -World.Landscape.LandscapeMaxX + BoundingVolumes.Dimensions.Z then begin
    Location.X := -World.Landscape.LandscapeMaxX + BoundingVolumes.Dimensions.Z;
  ...
  if Location.Z < -World.Landscape.LandscapeMaxZ + BoundingVolumes.Dimensions.Z then begin
    Location.Z := -World.Landscape.LandscapeMaxZ  + BoundingVolumes.Dimensions.Z;
