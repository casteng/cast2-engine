...
var XAxis, YAxis, ZAxis: TVector3s; Dist: Single;
begin
  SubVector3s(ZAxis, Location, LastLocation);

  if SqrMagnitude(ZAxis) < epsilon then ZAxis := GetVector3s(0, 0, BoundingVolumes.Dimensions.Z);
...
begin
  ...
  if CDObjects and not FirstProcess then begin
    ...
    for i := 0 to TotalItems-1 do if Items[i] <> Self then begin
      CR := VolumeColDet(ShellBoundingVolumes, Items[i].BoundingVolumes);
