...
type
  ...
  public
    ...
    procedure Pause(bPause: BOOL); virtual;
...
procedure CD3DApplication.ChangeDevice;
begin
  // Prompt the user to select a new device or mode
  if m_bActive and m_bReady then
  begin
    Pause(True);
    if FAILED(UserSelectNewDevice) then Exit;
    Pause(False);
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      if (m_Adapters[a].devices[d].bWindowed) then
      begin
...
//-----------------------------------------------------------------------------
procedure CD3DApplication.Pause(bPause: BOOL);
{$WRITEABLECONST ON}
const
...
begin
  ...
  m_bReady:= not (dwAppPausedCount > 0);
